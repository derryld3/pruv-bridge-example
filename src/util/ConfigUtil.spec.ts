...
describe('ConfigUtil', () => {
  ...
  describe('getRpcUrl', () => {
    it('should return the first RPC URL for a valid chain', () => {
      expect(ConfigUtil.getRpcUrl('ethereum')).toBe(
  ...
  describe('getRouterAddress', () => {
    ...
    it('should throw error for non-existent asset', () => {
      expect(() =>
        ConfigUtil.getRouterAddress('NONEXISTENT', 'ethereum'),
      ).toThrow("Asset 'NONEXISTENT' not found in configuration");
    });
  ...
  describe('getCollateralAddress', () => {
    ...
    it('should throw error for non-existent asset', () => {
      expect(() =>
        ConfigUtil.getCollateralAddress('NONEXISTENT', 'ethereum'),
      ).toThrow("Asset 'NONEXISTENT' not found in configuration");
    });

    it('should throw error for asset not available on specified chain', () => {
      expect(() =>
        ConfigUtil.getCollateralAddress('USDC', 'avalanche'),
      ).toThrow("Asset 'USDC' not available on chain 'avalanche'");
    });
  });
});
